...
\newcommand{\std}{0.3219} % (Standard deviation of acinar volumes), add "ddof=1" to get the same STD as with "=STDEV()" in Excel
...
%With the number of acini calculated above, this translates to a mean airspace surface of \SI{\meanairspacesurface}{\centi\meter\squared} (%
%animal 1: \SI{\airspacesurfaceB}{\centi\meter\squared},
%animal 2: \SI{\airspacesurfaceD}{\centi\meter\squared} and
%animal 3: \SI{\airspacesurfaceE}{\centi\meter\squared}).
...
\renewcommand{\imsize}{0.37\linewidth}
\begin{figure}[htb]
	\centering
	\subfloat{
	...
	\caption{Plot of the estimated acinar volumes of each sample, extracted with MeVisLab and estimated using the STEPanizer. The line corresponds to the mean, the dashed lines correspond to the mean \(\pm\biggerthan\times\) the standard deviation. Red dots represent outliers and were not used for calculating the mean. Top row: MeVisLab volumes, bottom row: STEPanizer volumes. From left to right: animal 1, 2 and 3.}
...
\begin{table}[htb]
	...
	Acinar volume [\si{\milli\meter\cubed}]		& \acinarvolumeB & \acinarvolumeD & \acinarvolumeE & \meanacinarvolume \\
	Acinar surface [\si{\milli\meter\squared}]	& \acinarsurfaceB & \acinarsurfaceD & \acinarsurfaceE & \meanacinarsurface \\
...
Our calculations resulted in a volume of \SI{\meanacinarvolume}{\milli\meter\cubed} with a standard deviation of \SI{\std}{\milli\meter\cubed}.
