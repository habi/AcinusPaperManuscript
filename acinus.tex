...
\usepackage{natbib} % \citep instead of \cite
\usepackage{scrtime}
\usepackage{xspace}
...
\begin{itemize}
	\item David Haberthür performed data acquisition, designed and performed analysis of data, estimated acinar volumes and surfaces and wrote the manuscript.\todo{drafted or wrote?}
...
Knorpel = Bronchien, conducting
ohne = Bronchioli, conducting
terminal bronch = letzte conductiing
transitory bronch = eingang zum acinus entspricht -> dort kommen die manhole covers
alveolar ducts = anfang vom acinus



The airway structure of the mammalian lung is formed from largely dichotomous branches~\citep{Weibel1991}\todo{Format Citations!}, both starting from the trachea.
...
After this point, the so-called acinar airways and all the following airspace form one functional lung unit, the so-called acinus. One acinus is composed of an alveolar duct leading into multiple saccules covered by alveoli which mark the terminal end of the airway tree~\citep{Schittny2007a}.
...
The present manuscript describes a method to isolate and analyze single acini from large synchrotron radiation based tomographic microscopy datasets~\citep{Haberthuer2010a} and to stereologically analyze biological parameters of those single functional units of the mammalian lung.
Since tomographic imaging of the lung tissue preserves the three-dimensional structure, single slices from the images can be easily extracted and be used for precise stereological analysis, e.g. the volume, surface area, and number of alveoli per acinus~\citep{Hsia2010}.

In addition to three-dimensional volume analysis through segmentation and voxel counting in a visualization software we performed standard stereological analysis~\citep{Hsia2010} to compare our results with both methods~\citep{Rodriguez1987,Tschanz2003}.
...
Tomographic imaging was performed at the \href{http://www.psi.ch/sls/tomcat/}{TOMCAT beamline} of the \href{http://www.psi.ch/sls/}{Swiss Light Source} at the \href{http://www.psi.ch/}{Paul Scherrer Institute} in Villigen, Switzerland~\citep{Stampanoni2006a}.
...
The resulting large tomographic datasets had a size of approximately 3000\(\times\)3000\(\times\)3072 pixels with \SI{1.48}{\micro\meter} pixel size.\todo{kürzen}
...
We extracted conducting airway segments using a threshold interval based region growing algorithm~\citep{Zucker1976}.
...
\todo{etwas ausbauen, evtl. sogar Figuren aus M\&M zitieren oder verschieben}
\todo{Segmentierung, Manhole covers und zählen auf *Tomographie* Schnitten}
...
For the three lung investigated we calculated \totalnumberofaciniB, \totalnumberofaciniD\xspace and \totalnumberofaciniE\xspace acini per lung, respectively, resulting in a mean of \meantotalnumberofacini\xspace with a standart deviation of \meantotalnumberofaciniSTD.

\subsection{Number of alveoli per acinus}
By counting appearing and disappearing bridges in the segmented acini, we found a mean number of alveoli for all analzyed acini of \meannumberofalveoli. (%
...
\todo{Letzter aber *nur* dann, wenn die Segmentierung korrekt und stimmig gemacht ist -> Letzten Absatz etwas umformulieren).}
