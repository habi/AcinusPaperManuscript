...
\ifJCS
	\documentclass[final,paper=a4,DIV=calc,abstract,english]{scrartcl}
		...
		\usepackage{natbib}
\else
	\documentclass[draft,paper=a4,twocolumn=true,DIV=calc,abstract,english]{scrartcl}
...
\newcommand{\meanacinarvolume}{1.148\xspace} % mm^3, (mean acinar volume)
\newcommand{\meanacinarvolumeSTD}{0.322\xspace} % (Standard deviation of acinar volumes), add "ddof=1" to get the same STD as with "=STDEV()" in Excel
...
\newcommand{\difference}{2.661\xspace} % X times bigger (acinar volumes STEPanizer/MeVisLab-volumes)
...
\newcommand{\meanacinarsurface}{73.9\xspace} % mm^2 (changed from cm^2 to mm^2 in ReadVolumeSurfaceAndAlveaolarNumber.py) http://is.gd/99Sa9v
...
\newcommand{\meanairspacesurface}{5891\xspace} % cm^2
...
\newcommand{\meantotallungvolume}{10397\xspace} % mm^3
...
The tomographic datasets obtained at TOMCAT were converted from a stack of TIFF-files to the native GVR format of MeVisLab, a multi-resolution \deleted[id=st]{\href{https://secure.wikimedia.org/wikipedia/en/w/index.php?title=Octree&oldid=409131920}{octree}-based} image format.
...
\added[id=st]{The volume was estimated according to the Cavalieri principle: profile area on equidistant serial sections of acini was estimated by simple point counting.
...
Alveolar number was estimated by counting the number of entrance rings in paired sections by using the disector\replaced[id=st]{~\citep{Sterio1984} accomplished with the STEPanizer (experimental feature).}{mode of the STEPanizer~\citep{Hsia2010}} (\autoref{fig:STEPanizer Eveline}).
...
The absolute number of acini per lung can also be estimated by dividing the estimated total number of alveoli \added[id=st]{in the entire lung} (\added[id=dh]{calculated based on} personal communication Stefan Tschanz and Johannes Schittny) by the number of alveoli per acinus.
...
\replaced[id=dh]{W}{The three-dimensional structure of the lung samples is preserved during the tomographic imaging, w}e can thus extract individual acini and assess all the biologically interesting parameters mentioned above for each acinus individually.
...
\replaced[id=st]{\citeauthor{Mercer1987a} assigned the the transitory bronchioles to the conducting airways.
Thus, the start of their acini was one generation deeper resulting in a volume about two times smaller than our acinus.
In that light our stereologically estimated mean acinar volume is in the range of both previously published data.}{If we take in account that \cite{Mercer1987a} measured the volume of ventilatory units that have been defined differently (they define the transitory bronchioles as conducting airways, \ie their acinus is about two times smaller than our acinus), our stereologically estimated mean acinar volume is in the range of previously published data.}

\subsection{Total number of acini}
We did not directly estimate the number of acini, but calculated this number by \added[id=st]{two different approaches: (i) by dividing the total number of alveoli (own unpublished data of Tschanz and Schittny) by the mean number of alveoli per acinus and (ii)} by dividing the total parenchymal airspace~\citep{Tschanz2003} \replaced[id=st]{by the mean accinar volume (see above)}{and the total number of alveoli (own unpublished data of Tschanz and Schittny) by the corresponding mean acinar values}.
However, \replaced[id=st]{these calculations are}{this calculation is} only valid if we assume the acini \replaced[id=st]{showing}{possess} a similar size throughout the different regions in the lung.

We found \added[id=st]{differing values with} a mean number of \meantotalnumberofaciniVariant acini per rat lung for our three animals by dividing the estimated total number of alveoli by the number of alveoli per acinus.
...
\subsection{Number of alveoli per acinus}
\replaced[id=st]{We found \(\meannumberofalveoli\pm\meannumberofalveoliSTD\) alveoli per acinus showing a considerable high deviation.
...
Their values of \(936\pm254\) in old mice was almost 9 times higher than our results.
...
\added[id=dh]{\citet{Vasilescu2012} found 3883 acini per lung in old mice, we found \meantotalnumberofaciniVariant acini for 60 days old rats, a value \(1.4\times\) %really 1.41
higher.
Based on the strains we studied (SV 127 mice versus Sprague-Dawley rats~\citep{Mund2008, Schittny2008}) an 60 days old rat possesses an \(20.1\times\) larger parenchymal volume than a mouse of the same age (8932~\citep{Tschanz2003} vs.\ \SI{444}{\cubic\milli\meter}~\citep{Mund2008}).
The acinar volume for old mice is \SI{0.36}{\cubic\milli\meter}~\citep{Vasilescu2012}, while we found \SI{\meanacinarvolume}{\milli\meter\cubed} for our rats, a value 
\(3.2\times\) larger.

\replaced[id=st]{On each level}{For each} of these parameters we see larger values for rat compared to mice.
We hypothesized that the additional volume of the rat lungs is filled with substantially more and slightly larger alveoli---resulting in a larger number of alveoli per acinus in rats.
Since the alveolar volume does not change substantially between rats and mice \added[id=js]{(approximately 2 to \(3\times\))}, we need to have substantially more alveoli to fill each acinus.
Indeed we observed a mean of \meannumberofalveoli alveoli per rat acinus, which is \(9\times\) the number of alveoli in mice acini (936\(\pm\)254 in old mice~\citep{Vasilescu2012}).

We see that the species difference lies above all in the relative volume change of the acini.
It would be very interesting to estimate how the acinar volume of humans fits into this relation.
}
