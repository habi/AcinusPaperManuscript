\documentclass[paper=a4,twocolumn=true,DIV=calc,abstract,english]{scrartcl}
...
\usepackage[switch]{lineno}
	\linenumbers
\usepackage{xspace}
\usepackage{setspace} % to double-space the references
\usepackage[autostyle=true]{csquotes}
\usepackage{microtype}
\usepackage{textcomp} % to suppress warnings from microtype
\usepackage{changes} % try using [final] if the visible changes bother you.
...
% Changes options
\definecolor{rev1}{RGB}{159,94,171}
\definecolor{rev2}{RGB}{149,189,86}
\definecolor{david}{RGB}{163,85,64}
\definecolor{johannes}{RGB}{126,154,153}
...
\definechangesauthor[name={Reviewer 1}, color=rev1]{rev1}
\definechangesauthor[name={Reviewer 2}, color=rev1]{rev2}
...
In humans the acini possess approximately four \added[id=js]{generations of} respiratory bronchioles each owing four \added[id=js]{generations of} alveolar ducts.
...
These are tube-like structures where the proximal part resembles the wall of a terminal \replaced[id=rev1]{bronchiole}{bronchioli} and the distal part an alveolar duct.
...
\citet{Rodriguez1987} claimed to have done the \blockquote{first attempted serial-section reconstructions} \added[id=dh]{of rat lung acini} \blockquote{but realized the limitations of this very tedious although precise method} and finally switched to casting of airways with silicon to assess the acini in rat and rabbits.
They analyzed acini of one animal for each species.

\citet{Mercer1987a} manually traced several \added[id=dh]{rat} acini on photographs of serial sections with a thickness of \SI{3}{\micro\meter} each.
...
\added[id=dh]{Other researchers have analyzed several murine acini, \eg \citet{Kumar2013} based on microCT data, while \citet{Sera2013} and \citet{Xiao2013} analyzed data obtained at the synchrotron radiation facility SPring-8 in Japan. In total, all these researchers semi-automatically extracted and analyzed 14 individual acini.}

Our study describes a method to isolate a large number of individual \added[id=dh]{rat lung} acini from synchrotron radiation based tomographic microscopy datasets in order to analyze their structural parameters like volume, surface and number of alveoli.
...
After fixation, the samples were prepared for tomographic imaging by post-fixation with \SI{1}{\percent} osmium tetroxide and staining with \SI{4}{\percent} uranyl acetate to increase the x-ray absorption contrast.
...
\added[id=dh]{Tissue shrinkage due to paraffin embedding has been measured with water displacement and subsequent point counting on equidistant serial sections~\citep[Cavalieri method]{Cruz-Orive1999}.}
...
As a result, the voxel \replaced[id=rev2]{side length}{size} was \SI{1.48}{\micro\meter} and the exposure time was between \SIrange{160}{200}{\milli\second}.
...
\added[id=dh]{Matching both the constraint in high resolution and large sample volume is relatively straightforward with the imaging capabilities of TOMCAT, as described below. Using a microCT imaging system, we would have needed to employ a cumbersome multi-step process (overview scan, segmentation of region of interest (ROI), cutting of the sample and subsequent high-resolution scan of this ROI) to achieve a comparable result, but with a lower resolution and contrast.}
...
The volume was estimated according to the Cavalieri principle: the profile area on equidistant serial sections of acini was estimated by simple point counting~\citep{Cruz-Orive1999}.
The volume was calculated by multiplying the area estimate by the distance of the serial sections \added[id=dh]{taking into account tissue shrinkage due to paraffin embedding}.
...
The absolute number of acini per lung can also be estimated by dividing the estimated total number of alveoli in the entire lung (Tschanz and Schittny, personal communication) by the number of alveoli per acinus.
For the three lung\added[id=js]{s} investigated we calculated \totalnumberofaciniBVariant, \totalnumberofaciniDVariant and \totalnumberofaciniEVariant acini per lung, respectively, resulting in a mean of \meantotalnumberofaciniVariant with a standard deviation of \meantotalnumberofaciniSTDVariant.
...
\begin{table*}
	...
	Total number of acini (\replaced[id=rev1]{volume}{surface} based calculation)	& \totalnumberofaciniB 			& \totalnumberofaciniD 			& \totalnumberofaciniE 			& \meantotalnumberofacini 			& \meantotalnumberofaciniSTD \\
...
\begin{figure*}[htb]
	...
	\caption{Plot of the estimated acinar volumes of each sample, extracted with MeVisLab (automatic method\added[id=js]{, top row}) and estimated using the STEPanizer (manual assessment\added[id=js]{, bottom row}).
		...
		\deleted[id=js]{Top row: Automatically assessed volumes, bottom row: Manually assessed volumes.}
...
These ventilatory end-units contain all alveoli aerated by a terminal \replaced[id=rev1]{bronchiole}{bronchus}.
...
\replaced[id=dh]{Methods for estimating the total number of ventilatory units (equivalent to acini in species without respiratory bronchioles) do exist~\citep{Wulfsohn2010}, but they only permit an estimation of the global number of acini.
This is due to the loss of}{Losing} the three-dimensional context of a supposed acinus on sections \added[id=dh]{which} prevents an unbiased assignment to an individual acinus.
...
\citet{Mercer1987a} stated that the total volume of ventilatory units, defined as \blockquote{alveoli + ducts} was \SI{0.522(56)}{\milli\meter\cubed} (mean of 8). % http://is.gd/qpMEV5
...
For methodical reasons the filling of a particular region of the lung is fluctuating to some \replaced[id=rev1]{extent}{extend} and furthermore the filling varies between different samples.
...
\begin{description}
	\item[Lung] Total lung volume in adult animals showed a difference of \(7.4\times\) from mice to rats (\SI{\meantotallungvolume}{\cubic\milli\meter} vs.\ \citep[\SI{1398}{\cubic\milli\meter}]{Vasilescu2012})\deleted[id=js]{ or, taking the parenchymal volume even \(20\times\) (23, 8932 mm3 vs.\ 14, 444 mm3)}.
	\item[Acinus] Comparing the volumetric values on acinar level resulted in 3.2, \added[id=dh]{7.5 and 7}\(\times\) larger acini in rats (\SI{\meanacinarvolume}{\cubic\milli\meter} vs.\ \citep[\SI{0.36}{\cubic\milli\meter}]{Vasilescu2012}\added[id=dh]{, \citep[\SI{0.15}{\cubic\milli\meter}]{Sera2013} and 
\citep[\SI{0.16}{\cubic\milli\meter}]{Kumar2013}}).
We found \meantotalnumberofaciniVariant acini per lung of 60 days old rats, while \citet{Vasilescu2012} found 3883 acini per lung of \blockquote{old} mice, a value \(1.4\times\) larger \added[id=dh]{while the other researchers do not mention the total number of acini per lung}.
...
\begin{table*}
	...
	\begin{tabular}{r c c c}
		...
		\midrule
		Age [\si{\day}]											& 60										& \blockquote{old}, \numrange{46}{69}\added[id=dh]{, 63, 84 \citep{Vasilescu2012,Knust2009,Sera2013,Kumar2013}} & \\
		Lung volume [\si{\cubic\milli\meter}]				& \meantotallungvolume					& 1398 \citep{Vasilescu2012}	& 7.44\\
		Mean acinar volume [\si{\cubic\milli\meter}]		& \meanacinarvolume						& \added[id=dh]{\numlist{0.36;0.153;0.164} \citep{Vasilescu2012,Sera2013,Kumar2013}} & \added[id=dh]{\numlist{3.19;7.50;7.00}}\\
...
On each level of these parameters we see larger values for rat compared to mice.

The lung volume \added[id=js]{and the acinar volume} changes substantially between the species (\added[id=js]{approximately} \(7\times\)) while the alveolar volume changes only slightly (\(1.5\times\)). We hypothesize that the additional volume of the rat lungs is filled with substantially more and slightly larger alveoli---resulting in a larger number of alveoli per acinus in rats (\(9\times\) more than in mice).
...
The threshold had to be chosen \replaced[id=rev1]{ensuring}{insuring} that neighboring acini are separated.
...
\begin{figure}
	...
	\hfill%
	\caption{\replaced[id=js]{S}{Typical s}ections of two acini showing large differences between the automatic and manual volume assessment.
...
We have only shown data for one time point in the lung development, but a stereological estimation of the presented parameters would also be possible for multiple time points during lung development or in different disease models like fibrosis or \replaced[id=rev1]{emphysematous}{emphysematic} lungs.
