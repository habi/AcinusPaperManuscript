...
The origin and direction of the surface normal of the manhole covers were semiautomaticall placed in the three-dimensional tomographic dataset using the aforementioned custom MeVisLab GUI.
...
The absolute number of acini per lung can also be estimated by dividing the estimated total number of alveoli \added[id=st]{in the entire lung} (\added[id=dh]{calculated based on} Stefan Tschanz and Johannes Schittny, personal communication) by the number of alveoli per acinus.
...
\replaced[id=dh]{W}{The three-dimensional structure of the lung samples is preserved during the tomographic imaging, w}e can thus extract individual acini and assess \replaced[id=ms]{several}{all the} biologically interesting parameters mentioned above for each acinus individually.
...
\added[id=st]{\citeauthor{Mercer1987a} assigned the the transitory bronchioles to the conducting airways.
Thus, the start of their acini was one generation deeper resulting in a volume about two times smaller than our acinus.}
\todo[inline]{Marco says: "Very interesting here, so who is right? You or \citeauthor{Mercer1987a}??" I would say we are correct, what do you think, Johannes?}
...
The alveolar volume in mice was found \SI{59500}{\cubic\micro\meter} \cite{Knust2009}, a value \(1.48\times\) larger.
%The alveolar volume in mice was found \SIlist{59500;250000}{\cubic\micro\meter} \cite{Knust2009,Vasilescu2013}, a value \numlist{1.48;0.35}\(\times\) larger.
...
\added[id=dh]{The lung volume changes substantially between the species (\(7\times\)) while the alveolar volume changes only slightly (\(1.5\times\)). We hypothesized that the additional volume of the rat lungs is filled with substantially more and slightly larger alveoli---resulting in a larger number of alveoli per acinus in rats (\(9\times\) more than in mice).}
...
\todo[inline]{Marco mentions: "I still find these sentences quite vague. Actually, your numbers are "strikingly" different from any previously published result and your arguments to bring these data through and to explain why they differ from published-work must be solid. Don't you think?"}
...
We thus expect the automatic volume estimation through grey level based threshold segmentation to show a larger variability \added[id=dh]{and to generally result in understimated volumes}.
