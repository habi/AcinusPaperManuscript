...
% Changes options
\definecolor{david}{RGB}{159,88,69}
\definecolor{rev1}{RGB}{144,186,109}
\definecolor{rev2}{RGB}{150,111,173}
...
\added[id=dh]{Matching both the constraint in high resolution and large sample volume is relatively straightforward with the imaging capabilities of TOMCAT, as described below. Using a microCT imaging system, we would have needed to employ a cumbersome multi-step process (overview scan, segmentation of region of interest (ROI) and subsequent high-resolution scan of this ROI) to achieve the same result.}
...
\replaced[id=dh]{Methods for estimating the total number of ventilatory units (equivalent to acini in species without respiratory bronchioles) do exist~\citep{Wulfsohn2010}, but they only permit an estimation of the global number for full samples.
