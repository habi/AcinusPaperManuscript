...
We found \meantotalnumberofaciniVariant acini per lung of 60 days old rats, while \citet{Vasilescu2012} found 3883 acini per lung of \blockquote{old} mice, a value \(1.4\times\) higher.
	\item[Alveoli] We calculated the mean alveolar volume by dividing the mean acinar volume by the mean number of alveoli per acinus. The mean acinar volume was corrected by \SI{65}{\percent}, the ratio of alveolar vs.\ ductal volume, calculated from \citep{Salm2013}.
The mean alveolar volume is \SI{88099}{\cubic\micro\meter} for rats. The alveolar volume in mice was found \SIlist{59500;250000}{\cubic\micro\meter} \cite{Knust2009,Vasilescu2013}, a value \numlist{1.48;0.35}\(\times\) larger.
...
\begin{table*}
	...
	\begin{tabular}{r S[table-format=5.3] S[table-format=4.3] S[table-format=1.2]}
		...
		% Total aleolar number Lilian	50.6 Mio
		Mean alveolar volume [\si{\cubic\micro\meter}]	& 88099									& \numlist{59500;250000}~\cite{Knust2009,Vasilescu2013}	& \numlist{1.48;0.35}\\
		\bottomrule
	\end{tabular}
	}
	\label{tab:volumes}
...
\replaced[id=st]{On each level}{For each} of these parameters we see larger values for rat compared to mice, the exception being the mean alveolar volume of mice found by \citet{Vasilescu2012}, which was found to be \(2.84\times\) larger in mice compared to rats.

\added[id=dh]{The alveolar number changes dramatically between the species (\numlist{20;28}\(\times\)), while the lung volume changes considerably (\(7\times\)) and the mean alveolar volume only slightly (\(1.5\times\)). We hypothesized that the additional volume of the rat lungs is filled with substantially more and slightly larger alveoli---resulting in a larger number of alveoli per acinus in rats (\(9\times\) more than in mice).}
