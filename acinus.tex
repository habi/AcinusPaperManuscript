...
Alveolar number was estimated by counting the number of entrance rings in paired sections by using the disector\replaced[id=st]{~\cite{Sterio1984} accomplished with the STEPanizer (experimental feature).}{mode of the STEPanizer~\citep{Hsia2010}} (\autoref{fig:STEPanizer Eveline}).
...
\deleted[id=dh,remark={moved to discussion}]{There are several methods for triangulation of three-dimensional surfaces~\citep{Lorensen1987}\citep{Schneiders1996}, nonetheless all these methods cannot overcome the problem of the coastline paradox. We thus did not assess the acinar surface \added[id=dh]{automatically} by triangulation (e.g. using MeVisLab).}
...
\subsection{Number of alveoli per acinus}
\added[id=dh]{The \meannumberofalveoli of alveoli per acinus we observed seems rather large.
Nonetheless we believe that the numbers are valid and in relation to other data.

\citet{Vasilescu2012} found 3883 acini per lung in old mice, we found \meantotalnumberofaciniVariant acini for 60 days old rats, a value \(1.4\times\) %really 1.41
higher.
...
For each of these parameters we see larger values for rat compared to mice.
...
Since the acinar volume does not change substantially between rats and mice we need to have substantially more alveoli to fill each acinus.
...
We see that the species difference lies above all in the relative volume change of the acini. It would be very interesting to estimate how the acinar volume of humans fits into this relation.
}
...
While various methods of triangulating a surface do exist, limitations in our computing power would have required a binning of the data which represents a reduction of the resolution.
There are several methods for triangulation of three-dimensional surfaces [Lorensen and Cline, 1987, Schneiders, 1996], nonetheless all these methods cannot overcome the problem of the coastline paradox.}

\added[id=dh]{We thus did not assess the acinar surface by triangulation, but used a a classic stereological way to estimate the surface area of the acini.}
