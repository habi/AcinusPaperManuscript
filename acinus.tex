...
The transitory bronchioles which follow mark the beginning of the gas-exchange region in the human lung, while in rat lungs this region is formed by the transitory bronchioles, which are tube-like structures covered with single alveoli.
...
One acinus is composed of an alveolar duct leading into multiple saccules covered by shaped alveoli which mark the terminal end of the airway tree~\citep{Schittny2007a}.
...
The resulting tomographic datasets covered a field of view of a cube with approximately \SI{4.5}{\milli\meter} sidelength (3000\(\times\)3000\(\times\)3072 voxels) with \SI{1.48}{\micro\meter} large voxels.
...
Alveolar number was estimated by counting the number of entrance rings in paired sections by using the disector mode~\citep{Hsia2010} of the STEPanizer.
...
These manhole covers acted as segmentation stoppers, enabled an automatedclosing of the airways tree and the following threshold based region growing segmentation permitted us to efficiently extract \numberofacini\xspace acini for further stereological analysis.
