\documentclass[draft,paper=a4,abstract=true,english,DIVcalc]{scrartcl}
%\documentclass[paper=a4,abstract=true,english,DIV=calc]{scrartcl}
...
\marginpar{Include a brief itemized list of how each author contributed to the study}
\begin{itemize}
...
By using high-resolution tomographic microscopic imaging we extracted single acini from rat lung samples to stereologically assess biological parameters.
...
Assuming that the acini are similarly sized throughout the different regions in the lung, we found that one rat lung contains approximately \meantotalnumberofacinirounded\xspace acini.
Based on the same assumption we calculated a mean airspace surface area of \SI{\meanairspacesurface}{\centi\metre\squared} per animal.
...
\section*{Keywords}
\marginpar{Three to five words that do not appear in the title or running head (which should be maximally 60 characters long, including spaces (the running head is shown at the top left of each page).}
\begin{itemize}
...
\marginpar{TOC is only for review purposes, it will not be in the final manuscript}
\tableofcontents
\clearpage
...
The present manuscript describes a method to isolate and analyze single acini from large synchrotron radiation based tomographic microscopy datasets~\cite{Haberthuer2010a} and to stereologically analyze biological paramaters of those single functional units of the mammalian lung.
...
Using a \href{http://python.org/}{Python} script, we extracted and calculated the volume assessed with MeVisLab and the STEPanizer for \numberofacini\xspace acini from their respective files, the results are presented in \autoref{sec:results} below.
...
The results are shown in \autoref{fig:Volume plots} and explained later on in the Discussion section.
...
\begin{figure}
	...
	\caption{Acinar volumes of each sample, extracted with MeVisLab and the STEPanizer. The dashed line corresponds to the mean mean, the dotted lines correspond to the mean \(\pm\biggerthan\times\) the standard deviation. Top row: MeVisLab volumes, bottom row: STEPanizer volumes. From left to right: animal B, D and E.}
...
\todo{\citet[Table 2]{Mercer1987a} state that the total volume of \blockquote{alveoli + ducts} is \SI{522(56)}{\micro\meter\cubed}$\times 10^6$, which corresponds to \SI{0.5}{\cubic\milli\meter}. % http://is.gd/qpMEV5
...
\citet{Mercer1987a} mention that differences in these number apperars to be due the different definition of the acinus, \ie that one ventilatory unit does not correspond to one acinus in their definition.
...
Even though our values are a only valid if we assume the acini to possess a similar size throughout the different regions in the lung, our results are comparable to previously published results, showing the feasibility of our method.
...
Since these automatically extracted regions were merely used as a optic guidance for the manual, stereological method it is obvious that the manual method shows larger but probably more precisely estimated volumes than the automatic method, which in contrast is by magnitudes faster.
...
\marginpar{Affiliation must reflect the organization(s) supporting the author(s) while the research was done. This may differ from the current affiliations of the author(s), which will be listed in such cases in the Acknowledgment section as the present address(es) of the author(s).}
...
\clearpage
\singlespacing
\marginpar{Reference lists should be arranged alphabetically by author and numbered serially. The reference number should be placed in parentheses at the appropriate place in the text.}
